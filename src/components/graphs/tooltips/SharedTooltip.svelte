<!--
 ...
 <script>
    ...
    import QuadTree from './QuadtreeTooltip.svelte';
    ...
    function sortResult(result) {
      if(result && Object.keys(result).length) {
        ...
        return allRows
          .map(d => ({ label: d[$config.z], value: d[$config.y] }))
  ...
  <QuadTree
    ...
    let:e
  >
    {@const tooltipData = sortResult(found)}
    {#if visible === true}
      <div
        style="left:{x}px;"
        class="line">
      ...
      <div
        ...
        >
          ...
          {#each tooltipData as row}
            <div class="row">
              <span class="key" style="--color: {colorMap.get(row.label)}">{formatKey(row.label)}:</span> {formatValue(row.value)}
            </div>
          {/each}
      </div>
    {/if}
